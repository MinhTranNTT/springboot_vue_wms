<template>
    <div style="text-align: center;background-color: #f1f1f3;height: 100%;padding: 0px;margin: 0px;">
        <h1 style="font-size: 40px;">{{'欢迎你！'+user.name}}</h1>
        <el-descriptions  title="个人中心" :column="2" size="40" border>
            ...
            <el-descriptions-item>
              ...
              <el-tag
                  ...
                  disable-transitions><i :class="user.roleId === 0 ? 'el-icon-user-solid' :
                  (user.roleId === 1 ? 'el-icon-s-custom' : 'el-icon-user')"></i>{{user.roleId === 0 ? '超级管理员' :
                  (user.roleId === 1 ? '管理员' : '用户')}}</el-tag>
...
<script>
    ...
    export default {
        ...
        methods:{
            init(){
                this.user = JSON.parse(window.sessionStorage.getItem('CurUser'))
            }
        },
        created(){
            this.init()
